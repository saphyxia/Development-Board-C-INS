...
#define BMI088_accel_write_single_reg(reg, data)    do { \
                                                    BMI088_ACCEL_NS_L(); \
                                                    BMI088_write_single_reg((reg), (data)); \
                                                    BMI088_ACCEL_NS_H(); \
                                                      }while(0U)

#define BMI088_accel_read_single_reg(reg, data)     do { \
                                                    BMI088_ACCEL_NS_L(); \
                                                    SPI_ReadWriteByte(&hspi1,(reg) | 0x80); \
                                                    SPI_ReadWriteByte(&hspi1,0x55); \
                                                    (data) = SPI_ReadWriteByte(&hspi1,0x55); \
                                                    BMI088_ACCEL_NS_H(); \
                                                      }while(0U)

#define BMI088_accel_read_muli_reg(reg, data, len)  do { \
                                                    BMI088_ACCEL_NS_L(); \
                                                    SPI_ReadWriteByte(&hspi1,(reg) | 0x80); \
...
#define BMI088_gyro_write_single_reg(reg, data)     do{ \
                                                    BMI088_GYRO_NS_L(); \
                                                    BMI088_write_single_reg((reg), (data)); \
                                                    BMI088_GYRO_NS_H(); \
                                                      }while(0U)

#define BMI088_gyro_read_single_reg(reg, data)      do { \
                                                    BMI088_GYRO_NS_L(); \
                                                    BMI088_read_single_reg((reg), &(data)); \
                                                    BMI088_GYRO_NS_H(); \
                                                      }while(0U)

#define BMI088_gyro_read_muli_reg(reg, data, len)   do{ \
                                                    BMI088_GYRO_NS_L(); \
                                                    BMI088_read_muli_reg((reg), (data), (len)); \
                                                    BMI088_GYRO_NS_H(); \
                                                      }while(0U)
...
//	mpu_data.gz_offset=mpu_data.gz_offset / 1000;
	...
	mpu_data.ax_offset=8727;
	mpu_data.ay_offset=3;
	mpu_data.az_offset=-1;
	mpu_data.gx_offset=3;
...
static void BMI088_write_single_reg(uint8_t reg, uint8_t data)
{
    ...
    SPI_ReadWriteByte(&hspi1,reg);//先将地址写入传感器
    SPI_ReadWriteByte(&hspi1,data);//在将寄存器值写入传感器
}

static void BMI088_read_single_reg(uint8_t reg, uint8_t *return_data)
{
    SPI_ReadWriteByte(&hspi1,reg | 0x80);  //这里为什么要单独与上0x80呢，因为手册里面提到，载陀螺仪模式下，要触发新的数据中断，通过将0x80写入寄存器去触发，其余模式也是一样的
    *return_data = SPI_ReadWriteByte(&hspi1,0x55);
}

//static void BMI088_write_muli_reg(uint8_t reg, uint8_t* buf, uint8_t len )
//{
//    SPI_ReadWriteByte(&hspi1, reg );
//    while( len != 0 )
//    {

//        SPI_ReadWriteByte(&hspi1, *buf );
//        buf ++;
//        len --;
//    }
...
static void BMI088_read_muli_reg(uint8_t reg, uint8_t *buf, uint8_t len)
{
    ...
    while (len != 0)
    {
        ...
        *buf = SPI_ReadWriteByte(&hspi1,0x55); 
        buf++;
        len--;
    }
}
